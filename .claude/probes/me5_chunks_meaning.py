...
print(
    f"CHUNK_WORDS={CHUNK_WORDS} RELAXED_SIM_BAR={RELAXED_SIM_BAR} CON_BAR={DEFAULT_CONTRADICTION_BAR} ENT_FLOOR={DEFAULT_ENTAILMENT_FLOOR}"
)
...
print(
    f"[P1] k_expected={k_expected} n_chunks={len(chunks)} total_a={ta} total_b={tb} "
    f"coverage_a={ta == len(src.split())} coverage_b={tb == len(rw.split())} "
    f"ordered_a={recon_a} ordered_b={recon_b}"
)
for i, (a, b) in enumerate(chunks):
    print(
        f"  chunk{i}: a={len(a.split())}w b={len(b.split())}w  a_head={a.split()[:4]} b_head={b.split()[:4]}"
    )
...
def show(tag, src_t, rw_t):
    ...
    print(
        f"[P2] {tag}: sim={sim:.4f} con={con} ent={ent} -> meaning_preserved={verdict} "
