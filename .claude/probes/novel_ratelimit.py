...
print(
    f"    exactly 3 pass then block: {r[:3] == [None] * 3 and r[3] is not None and r[4] is not None}"
)
...
print(
    f"    separate buckets: {r1 is None and r2 is None and r3 is None and r4 is not None and r5 is None}"
)
...
for i in range(A._RATE_BUCKET_SOFT_CAP + 5):
    A._rate_buckets[f"k{i}"] = (
        now0 - i * 0.001,
        1,
    )  # increasing recency: k0 oldest, k{cap+4} newest
...
all_old_dropped = (
    all(
        A._rate_buckets.get(k, (1e18, 0))[0] < A._rate_buckets[k2][0]
        for k in dropped
        for k2 in A._rate_buckets
    )
    if dropped
    else False
)
print(
    f"(8) soft cap {A._RATE_BUCKET_SOFT_CAP}: dropped {len(dropped)} (expect 5), oldest-evicted: {len(dropped) == 5}"
)
