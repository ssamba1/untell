...
for n in (5, 10, 20, 38, 39, 40, 41, 50):
    ...
    print(
        f"n={n:3d} score={s if s is None else round(s, 4)} rep={round(rep, 4)} common={round(common, 4)} burst={burst if burst is None else round(burst, 4)} nsents={len(nonempty)}"
    )
