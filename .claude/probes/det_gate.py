...
def fake_score(text, **kw):
    ...
    return {
        "max": 0.20,
        "mean": 0.15,
        "detectors": {"perplexity_burstiness": 0.20, "mage": 0.10},
        "tier": "lite",
        "scored": True,
        "flagged": False,
    }
...
try:
    ...
    r0 = R.untell_text(
        "Some input text long enough to score cleanly here.", tier="lite", max_iters=2, seed=1
    )
    calls["n"] = 0
    # gate: mage must be < 0.05, but it's 0.10 -> vetoed
    r1 = R.untell_text(
        "Some input text long enough to score cleanly here.",
        tier="lite",
        max_iters=2,
        seed=1,
    ...
    out = {
        ...
        "veto_worked": r0.get("stopped") == "passed"
        and r1.get("stopped") != "passed"
        and r2.get("stopped") == "passed",
    }
finally:
    R.score_text = orig
