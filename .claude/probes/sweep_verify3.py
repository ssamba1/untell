...
for n in range(1, 12):
    for k in range(0, 61):
        ...
        if len(out) != n:
            viol.append(f"len n={n} base={base}")
        if len(set(round(v, 6) for v in out)) != n:
            viol.append(f"dupe n={n} base={base}: {[round(v, 4) for v in out]}")
        if any(v < 0.4 - 1e-9 or v > 1.0 + 1e-9 for v in out):
            viol.append(f"range n={n} base={base}")
        if not any(abs(v - base) < 1e-6 for v in out):
            viol.append(f"base missing n={n} base={base}: {[round(v, 4) for v in out]}")
