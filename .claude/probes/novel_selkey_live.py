...
orig_select = (
    composite.CompositeRewriter.select if hasattr(composite.CompositeRewriter, "select") else None
)
print(f"has select method: {orig_select is not None}")
print(
    f"composite attrs: {[a for a in dir(composite.CompositeRewriter) if not a.startswith('_')][:12]}"
)
