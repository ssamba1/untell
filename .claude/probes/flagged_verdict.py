...
docs = {
    ...
    "ai": (
        "Moreover, the framework leverages a robust approach to delivery at scale. Furthermore, "
...
for name, d in docs.items():
    ...
    vt = _verdict_threshold(
        s.get("threshold", DEFAULT_THRESHOLD), s.get("detectors", {}), s.get("detector_modes", {})
    )
    correct = s["flagged"] == (s["max"] >= vt)
    out[name] = {
        "max": round(s["max"], 4),
        "flagged": s["flagged"],
        "verdict_threshold": round(vt, 4),
