...
if "tells" in tool_fns:
    r = tool_fns["tells"](
        "Moreover, the framework leverages robust solutions.", include_matches=True
    )
...
if "score" in tool_fns:
    ...
    out["bad_tier_refused"] = (
        isinstance(r, dict)
        and "error" in str(r.get("error", "")).lower()
        or "tier" in json.dumps(r).lower()
    )
