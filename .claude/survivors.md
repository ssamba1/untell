...
# Rows below rebuilt 2026-08-15 from me2 worktree HEAD (564) + sibling-unique (45)
...
| untell/scripts/sentences.py | 93 | logic: != -> == | `if modes.get("perplexity_burstiness") != "stdlib":` | KILLED by tests/test_stdlib_mode_is_uninformative.py: stdlib mode + lite tier (pb-only detector) -> original True (stdlib ranking uninformative, measured 0.493 AUROC / 91-of-100 at 0.250); mutant == short-circuits to "informative" (False). Prior 'stdlib path not exercised' note wrong — the modes dict is a direct parameter. Red on mutation, green on original. |
| untell/scripts/sentences.py | 163 | boundary: < -> <= | `if len(scores) < _MIN_SENTENCES_FOR_SPREAD:` | KILLED by tests/test_spread_at_bar_is_rankable.py (test_exactly_min_sentences_proceed_to_spread): exactly 3 rows (== _MIN_SENTENCES_FOR_SPREAD) with spread 0.049 -> original proceeds to the spread check (True, unrankable); mutant <= short-circuits to False (wrongly rankable). Prior 'corpus doesn't have exactly 3 at the boundary' note wrong — rows is a direct parameter. Red on mutation, green on original. |
...
| untell/scripts/sentences.py | 165 | boundary: < -> <= | `return (max(scores) - min(scores)) < _TARGETING_SPREAD_BAR` | KILLED by tests/test_spread_at_bar_is_rankable.py (test_spread_at_bar_is_rankable): exact float spread 0.05 (0.0 to 0.05) -> original False (rankable, not strictly below the bar); mutant <= True (unrankable at the bar). Prior 'scores have sufficient spread to cross bar' note wrong — the exact boundary is constructible. Red on mutation, green on original. |
...
| untell/detectors/perplexity_burstiness.py | 211 | UNKILLABLE: TTR window tuning constant constant: 100 -> 101 | `_TTR_WINDOW = 100` | KILLED by tests/test_ttr_window_is_100_words.py: 25 types x4 + one repeat (101 words) -> first-100 TTR 0.25 (>= floor 0.25, signal 0.0 under original); mutant includes word 101 -> 25/101 = 0.2475 < floor, repetition signal fires (0.0165). Prior 'tuning constant' UNKILLABLE note wrong — the window edge crosses the floor. Red on mutation, green on original. |
...
| untell/detectors/mage.py | 65 | logic: and -> or | `if not (i2l and all(isinstance(v, str) for v in i2l.values())):` | KILLED by tests/test_empty_id2label_is_rewritten.py: config with NO id2label -> original rewrites to {"0":"machine","1":"human"}; mutant (or) evaluates all() over empty as True -> rewrite SKIPPED, config stays broken and the model fails validation. Same mocked-load seam as row 64. Prior 'needs live model dir' note wrong. Red on mutation, green on original. |
| untell/detectors/mage.py | 69 | constant: 2 -> 3 | `raw["num_labels"] = 2` | KILLED by tests/test_empty_id2label_is_rewritten.py (same test as row 65): the rewrite must set num_labels=2 to match the 2-entry id2label; mutant 3 writes a 3-class scheme for a 2-label map -> forward/validation mismatch. Verified red on this mutation, green on original. |
...
| untell/detectors/mage.py | 112 | logic: or -> and | `if "machine" in str(v).lower() or str(v).lower() in ("label_0", "ai", "fake")` | KILLED by tests/test_ai_index_uses_machine_label.py: stub 3-class model (machine/human/neutral), logits [-5,2,2] -> original resolves ai_idx=0, score 0.0005 (P(machine)); mutant and is unsatisfiable, falls to human_idx -> 1-P(human)=0.5002. 2-class hides it (complementary), 3-class exposes it. Prior 'needs live model.config' note wrong. Red on mutation, green on original. |
| untell/detectors/mage.py | 121 | logic: or -> and | `if "human" in str(v).lower() or str(v).lower() in ("label_1", "real")` | KILLED by tests/test_human_index_resolves_from_label_1.py: stub model with id2label {"0":"spam","1":"label_1"} -> original resolves human_idx=1 via the label_1 fallback, score 0.018; mutant and is unsatisfiable -> both indices None -> score() returns None (detector silent). Prior 'needs live model.config' note wrong. Red on mutation, green on original. |
| untell/detectors/mage.py | 127 | constant: 700 -> 701 | `p = windowed_max(text, lambda w: 1.0 - float(_probs(w)[human_idx]), 700)` | KILLED by tests/test_mage_window_is_700_words.py (same construction as 129): 1401-word text, stub model scoring only a 701-word piece -> original 0.0025 (no 701 piece under window 700), mutant 0.998. Prior 'model-dependent' note wrong — the size-sensitive stub seam pins the constant. Red on mutation, green on original. |
| untell/detectors/mage.py | 129 | constant: 700 -> 701 | `p = windowed_max(text, lambda w: float(_probs(w)[ai_idx]), 700)` | KILLED by tests/test_mage_window_is_700_words.py: 1401-word text + size-sensitive stub model -> original 0.0025 (pieces 700/700/1, no 701-piece exists), mutant 701 pieces 701/700 -> 0.998. The window constant is exactly observable through score(). Red on mutation, green on original. |
...
| untell/detectors/commercial.py | 233 | boundary: < -> <= | `if _CL_TOKEN["token"] and time.time() < _CL_TOKEN["exp"]:` | KILLED (or variant) by test_commercial_mutation_guards AND tests/test_expired_copyleaks_token_is_refreshed.py: expired token -> original re-auths (fresh), mutant or returns the stale token with no call (48h token used past life -> API 401s). (<= variant: exact-expiry instant is a race, unkillable.) |
| untell/detectors/commercial.py | 233 | logic: and -> or | `if _CL_TOKEN["token"] and time.time() < _CL_TOKEN["exp"]:` | KILLED by tests/test_expired_copyleaks_token_is_refreshed.py: expired cached token -> original re-auths (fresh via spy), mutant or returns the stale token with no post call. Red on mutation, green on original. |
...
## 2026-08-14 me2 worker rewriter sweep: base.py (Anthropic wrapper - partial, baseline 5m key-gated tests)
...
## 2026-08-14 me2 worker rewriter sweep: targeted.py (11/15 pre-killed by suite, 4 survivors)
...
## 2026-08-14 me2 worker rewriter sweep wave 2: composite.py _intensity_sweep kills (diff-proven by swarm-a, materialized + verified here)
...
| eval/detector_audit.py | 284 | logic: and -> or | `elif au is not None and au < WEAK_AUROC:` | KILLED by tests/test_separated_detector_is_not_weak.py: stub detector with human [0.1,0.2] / ai [0.7,0.8] -> au 1.0, fpr 0.0, original OK_SEPARATED; mutant or makes the branch fire whenever au is present, downgrading a perfectly-separated detector to WEAK. Red on mutation, green on original. |
...
| eval/detector_audit.py | 398 | boundary: >= -> > | `out += [s for s in split_sentences(para) if len(s.split()) >= 10]` | Same kill as the constant row above: tests/test_ten_word_sentences_become_probes.py (exactly-10-word sentences excluded under mutant -> probe fallback). Duplicate row, same verdict. |
| eval/detector_audit.py | 495 | logic: and -> or | `and r["auroc"] > SENTENCE_BROKEN_AUROC` | KILLED by tests/test_near_chance_sentence_detector_not_excused.py: sentence detector with verdict INVERTED + auroc 0.1 (< bar 0.2) -> original keeps it in broken (no 'Not counted'); mutant or bypasses the AUROC guard whenever auroc is present, excusing the near-chance detector. Red on mutation, green on original. |
...
| untell/api_server.py | 496 | logic: or -> and | `retry_after = _rate_limited(request, x_key or auth or "")` | KILLED by tests/test_rate_limit_keyed_by_api_key.py: captured credential passed to _rate_limited is 'secret' (the caller's X-API-Key) under original; mutant `x_key and auth and ""` always yields "" — every credentialed caller lands in the anonymous bucket, so one client's flood throttles all others. Red on mutation, green on original. |
...
| untell/rewriter/structural.py | 2516 | boundary: > -> >= | `key=lambda i: (0 if counts.get(first_words[i], 0) > 1 else 1, random.random()),` | KILLED by tests/test_opener_budget_prefers_duplicates.py: actual line 2503 (post-refactor). Sentences with dup 'The' openers + 'Zebra' (lowercase 'zebra' elsewhere -> eligible) + budget 1: 40-seed sweep -> original 0/40 picks the non-duplicate, mutant 11/40 (count>=1 makes single-occurrence openers eligible; the budget randomly lands on non-duplicates the transform exists to fix). Red on mutation, green on original. |
...
| untell/text_split.py | 57 | constant: 6 -> 7 | `if not word or len(word.replace(".", "")) > 6 or any(len(p) > 1 for p in parts):` | Cap raised 3->4 (pass 2026-08-15 slice4, U.S.S.R.) then 4->6 (U.N.E.S.C.O./U.N.I.C.E.F., 6 letters). 6->7 needs a 7-letter dotted initialism; nothing real exists. Unkillable; pinned at 6 by tests/test_four_letter_dotted_initialism_is_an_abbreviation.py. |
| untell/text_split.py | 57 | logic: or -> and | same | `not word` is dead: a fragment ending in "." always has a non-empty word. Unreachable through split_sentences. |
| untell/text_split.py | 103 | constant: False -> True | `if not tail.endswith("."): return False` | Gated unreachable: _continues_after_abbreviation requires ends_with_abbreviation first, which requires tail.endswith("."). |
| untell/text_split.py | 170 | constant: False -> True | `return False` (end of _first_alpha_is_lower) | Fires only on a continuation with NO letters or digits at all (degenerate). |
| untell/text_split.py | 244 | constant: 6000 -> 6001 | `_EXACT_ALIGN_LIMIT = 6000` | Boundary shift; a killing test needs a 6001-word pair through the exact difflib path (~18s+ per run). Perf tuning, both sides stay bounded. |
| untell/text_split.py | 245 | boundary: > -> >= | `if longest > _EXACT_ALIGN_LIMIT:` | Same family as 244. |
| untell/text_split.py | 273 | logic: and -> or | `if ca.strip() and cb.strip():` | Empty-chunk filter; proportional cuts cannot produce an empty chunk (k <= min(len) always), difflib-path duplicates are anchored/monotone. Unreachable in practice. |
| untell/layout.py | 188 | logic: and -> or | `if fence is None and line.strip() == "$$":` | KILLED by tests/test_layout_fence_math_state_isolation.py (slice4): the or-mutant toggles in_math on a $$ inside a fence, locking the prose after it. Red on mutation (7 failed), green on original. |
| untell/layout.py | 170 | logic: and -> or | `if not in_math: if fence is None: ...` | KILLED by tests/test_layout_fence_math_state_isolation.py (slice4): the or-mutant opens a fence on a ``` inside a math block, leaking fence state past the closing $$. |
