...
| untell/scripts/quality.py | 214 | identity: is not -> is | `if cos is not None:` | KILLED by tests/test_similarity_uses_embedding_when_available.py: similarity('cat','dog') -> 0.66 (embedding cosine) under original, 0.0 (token overlap, no shared tokens) under mutant — the backend silently swaps and the 0.76 gate bar lives on the raw-cosine scale. Test skips when no embedding backend. Red on mutation, green on original. |
