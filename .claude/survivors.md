...
| untell/text_split.py | 152 | boundary: <= -> < | `if blk.a <= i < blk.a + blk.size:` | KILLED by tests/test_cut_at_block_start_maps_through_block.py: fake difflib matcher with block at a=50; 100-word pair cut at i=50 (exact block start) -> 2 chunks of 50 under original, 1 chunk of 50 under mutant (i==blk.a skips the block, anchors to 100, second chunk empty -> filtered). Actual line is 183. Red on mutation, green on original. |
...
| untell/scripts/quality.py | 78 | constant: True -> False | `_bs_model = BERTScorer(lang="en", rescale_with_baseline=True)` | KILLED by tests/test_bertscore_uses_rescaled_baseline.py: monkeypatched bert_score.BERTScorer captures kwargs -> rescale_with_baseline must be True; mutant passes False. Also observable in scores: same pair F1 0.9348 rescaled vs 0.9890 raw. Prior 'only affects BERTScore which is NOT the gate' note misses that the constructor contract is still pinned. Red on mutation, green on original. |
...
| untell/scripts/io_utils.py | 290 | constant: False -> True | `interactive = False` | KILLED by tests/test_broken_isatty_still_reads_piped_stdin.py: monkeypatched stdin whose isatty raises + read()='piped content' -> original returns 'piped content', mutant returns None (drops piped input). The comment says the fallback exists so piped input still reaches the command. Prior 'branch never exercised' note wrong — the branch is forceable. Red on mutation, green on original. |
...
| untell/scripts/voice.py | 253 | constant: True -> False | `p.add_argument("--sample", required=True, help="file of YOUR writing (120+ words` | KILLED by tests/test_voice_cli_requires_sample.py: main(['--draft', PATH]) -> SystemExit(2) under original, TypeError (os.path.exists(None)) under mutant — the required guard keeps the usage-error contract instead of a crash. Red on mutation, green on original. |
| untell/scripts/voice.py | 265 | constant: 2 -> 3 | `print(json.dumps(report, ensure_ascii=True, indent=2))` |
| untell/scripts/verify.py | 139 | constant: False -> True | `results[d.name] = {"ai": None, "passes": False, "error": "detector returned NaN"}` | KILLED by tests/test_nan_detector_is_not_a_pass.py: fake detector returning float('nan') -> row {ai: None, passes: False, error: 'detector returned NaN'} under original, passes:True under mutant (un-scored text reads as clean). Red on mutation, green on original. |
| untell/scripts/verify.py | 172 | constant: 4 -> 5 | `"ai": round(ai, 4),` |
| untell/scripts/verify.py | 177 | constant: False -> True | `results[key] = {"ai": None, "passes": False, "error": str(exc)[:160]}` | KILLED by tests/test_raising_browser_checker_is_not_a_pass.py: monkeypatched untell.browser_check.get_browser_checker returns a checker whose check() raises -> row {ai: None, passes: False, error: 'boom'} under original, passes:True under mutant. The hook is imported function-locally so it patches at untell.browser_check. Red on mutation, green on original. |
...
| untell/layout.py | 91 | logic: == -> != | `if len(mask) == len(src):  # a classifier/line disagreement is not something to ` |
| untell/scripts/numerals.py | 214 | logic: or -> and | `return trimmed or "0"` | KILLED by tests/test_canonical_strips_trailing_zeros.py: _canonical('5.50') -> '5.5' original, '0' mutant (trimmed and '0' collapses every decimal to '0', breaking the string-comparison canonicalization the docstring documents). Red on mutation, green on original. |
...
| untell/layout.py | 226 | logic: or -> and | `if not buffer and (line.startswith("    ") or line.startswith("\t")):` | KILLED by tests/test_indented_line_after_blank_is_layout.py: 'Para one.\n\n    indented code\nMore prose.' -> original keeps the indented line as layout (locked), mutant gathers it into the prose block ('    indented code\nMore prose.') and it becomes transformable. The and-mutant is impossible (a line can't start with both 4 spaces and a tab). Red on mutation (2 failed), green on original. |
...
| untell/scripts/quality.py | 214 | identity: is not -> is | `if cos is not None:` | KILLED by tests/test_similarity_uses_embedding_when_available.py: with _cosine_similarity pinned to 0.5 via monkeypatch, original returns 0.5 (clamped cosine), mutant falls to token_overlap -> 0.0. The 0.76 gate bar lives on the raw-cosine scale so the backend swap is not scale-invariant. No skip (guard envelope); deterministic. Red on mutation, green on original. |
| untell/scripts/quality.py | 291 | constant: 2 -> 3 | `return 2` | KILLED by tests/test_quality_cli_missing_args_exits_two.py: main([]) and main(['only-one']) -> 2 under original, 3 under mutant. Usage-error convention (docstring documents the -h fix history). Actual line is 307; the 291 row predates a docstring insert. Red on mutation, green on original. |
