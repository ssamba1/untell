...
## WAVE 7 — RED / decision items

WHAT   `untell-audit --fix-counts` still needs one human run. Count drift is now reported as DRIFT
       rather than FAIL (#20, commit 2a64a7b), so it no longer holds the derivable-checks test red
       — but the published comparative counts in docs/why-best-open-repo.md and
       docs/humanizer-census.md remain stale.
RAN    untell-audit  (via slice 3)
SAW    failures=[]  count_drifts=["every 'N test modules' claim matches tests/",
       "every 'N tests' claim is close to what pytest collects"]
WHY    Rewrites PUBLISHED numbers in shipped docs — RED by the envelope, never an agent's call.
NEXT   One human run of `untell-audit --fix-counts`, then commit the regenerated docs.
NOTE   Wave 6 left pre-staged --fix-counts edits uncommitted in the working tree. They are no longer
       present (tree is clean, and `git diff a1196f1..HEAD` shows no change to those two files).
       Nothing hand-authored was lost: those edits were GENERATED by --fix-counts, so the single
       command above reproduces them. Recorded rather than quietly re-created, because an edit that
       vanished should be visible in the log even when it costs nothing.

WHAT   Docker image never live-tested (#50).
RAN    docker build (slice 16)
SAW    No Docker daemon available in this environment. 9/9 static checks pass (multi-stage layout,
       stdlib healthcheck, build context correctness).
WHY    Cannot claim a passing image from a static read.
NEXT   Human runs the build + healthcheck on a machine with a daemon; #50 tracks it.

WHAT   Named-signal rewrite rubric (#3, commit 590c115) is shipped but UNMEASURED, and reaches only
       users with an API key (#46).
RAN    grep -rn build_rewrite_prompt untell/
SAW    Two call sites, both hosted rewriters (Anthropic, OpenAI). The free rewriters consume no
       prompt. With no key configured, neither the in-sample delta nor the held-out RADAR check runs.
WHY    Publishing an effect size for an untested capability would be a fabricated number.
NEXT   Whoever holds a key runs the acceptance command in ~/goals/results/w7-06.md. Until then the
       rubric is implemented, tested for shape, and unproven for effect.
