...
|---|---|---|---|
| `threshold` | `0.30` | Stop when the **max** detector P(AI) drops below this — the loop's stop target (`flagged` is decided by `verdict_threshold`, which is `0.45` on the stdlib lite path — see "max is a stop target, not a verdict" below). | Measured on 40 labelled HC3 pairs: catches 100% of AI at every value tested, and reports 5% (lite) / 12% (full) of *human* documents as AI. See "What each threshold actually costs" below — 0.40 is the better pure-detection bar; 0.30 is kept because it is also the loop's stop target. |
| `similarity bar` | `0.76` embedding / `0.50` token-overlap | A rewrite is only acceptable if similarity to the original stays ≥ this. **Metric-aware** — the two metrics live on different scales. | `0.76` is the P-SP threshold (semantic embeddings). BERTScore had a third bar (`0.88`) and is **gone**: measured 2026-08-09 every meaning-CHANGED rewrite scored ABOVE every faithful one (0.853–0.958 vs 0.800–0.841), so no threshold separates them, and as the gate it rejected 19 of 20 real rewrites — `method()` never returns `bertscore` and `0.88` is never emitted (see `scripts/quality.py`). The lite token-overlap fallback scores faithful paraphrases far lower, so it uses `0.50` and is treated as *advisory* (`confidence: low`) — it cannot actually judge meaning. `quality.py` reports the right bar + `passes`; trust those, never a remembered number. |
