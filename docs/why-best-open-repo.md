...
| (d) **user-installable package** | `pip install` + **27** console scripts (`untell`, `-score`, `-loop`, `-humanize`, `-verify`, `-prove`, `-sentences`, `-tells`, `-voice`, `-humanness`, `-hedges`, `-numbers`, `-scrub`, `-explain`, `-batch`, `-watch`, `-compare`, `-ceiling`, `-detector-audit`, `-head-to-head`, `-eval-policy`, `-distill`, `-surrogate`, `-server`, `-mcp`, `-audit`, `-latex`) **and** an MCP server, a REST API and a Claude skill |
...
| Automated tests | ✅ **9214** tests, 594 modules (reproduce with `UNTELL_LITE_NO_TORCH=1 pytest --collect-only -q`) | ◑ sanity | ✅ | ✅ | ❌ | ❌ | ◑ manual |
