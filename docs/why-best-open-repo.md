...
| (d) **user-installable package** | `pip install` + **28** console scripts (`untell`, `-score`, `-loop`, `-humanize`, `-verify`, `-prove`, `-sentences`, `-tells`, `-voice`, `-humanness`, `-hedges`, `-numbers`, `-scrub`, `-explain`, `-batch`, `-watch`, `-compare`, `-ceiling`, `-detector-audit`, `-subgroup-audit`, `-ngram-lm`, `-eval-policy`, `-distill`, `-surrogate`, `-server`, `-mcp`, `-audit`, `-latex`) **and** an MCP server, a REST API and a Claude skill |
