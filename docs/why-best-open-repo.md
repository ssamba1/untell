...
| (d) **user-installable package** | `pip install` + **30** console scripts (`untell`, `-score`, `-loop`, `-humanize`, `-verify`, `-prove`, `-sentences`, `-tells`, `-voice`, `-humanness`, `-hedges`, `-numbers`, `-scrub`, `-explain`, `-batch`, `-watch`, `-compare`, `-ceiling`, `-detector-audit`, `-subgroup-audit`, `-detector-calibration`, `-ngram-lm`, `-gpt2-ppl`, `-eval-policy`, `-distill`, `-surrogate`, `-server`, `-mcp`, `-audit`, `-latex`) **and** an MCP server, a REST API and a Claude skill |
...
| Automated tests | ✅ **9249** tests, 593 modules (reproduce with `UNTELL_LITE_NO_TORCH=1 pytest --collect-only -q`) | ◑ sanity | ✅ | ✅ | ❌ | ❌ | ◑ manual |
