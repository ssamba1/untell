...
|---|---|---|
| **Local open-source** (RoBERTa-OpenAI, Fast-DetectGPT, Binoculars, MAGE) | Yes — runs locally on CPU/GPU | **Trained:** 97.6% ASR, mean TPR@1%FPR → 0.024 (StealthRL). **Inference-only (untell's regime): now MEASURED, and the answer depends on the corpus** — see [`free-ceiling-measured.md`](free-ceiling-measured.md), which opens with that warning. On the built-in demo corpus (three hand-written paragraphs, 37 words) mean max P(AI) goes **0.859 → 0.154 ± 0.042 at `--repeats 9`, flagged 1.00 → 0.00**. On real HC3 ChatGPT answers (195 words), the same loop and settings give **0.999 → 0.860 ± 0.001, flagged 1.00 → 1.00** — not one sample cleared. Meaning is held by an NLI gate in both. This row previously quoted **0.86 → 0.21, flagged 1.00 → 0.11**: a superseded figure, from the demo corpus, with no corpus named and no spread — the same mistake the README was corrected for in 2c4a6fb. ⚠️ This row previously read *"a modest, unreliable drop (0.86 → ~0.74–0.76) … with content-locked detectors immovable by any meaning-preserving rewrite"* — **both halves were falsified by re-measurement.** The weak figure came from taking a single rewrite draw (best-of-N selection was the shipped default at `1`), and the "immovable" content detector reaches **0.020** once selection and the meaning gate are fixed. Closer to the trained ceiling than this report originally concluded, though still on local proxies only. |
