...
FIXTURE = ROOT / ".claude" / "probes" / "census-2026-09-01-ai-humanizer.json"
...
def test_the_fixture_is_a_real_harvest():
    ...
    assert len(REPOS) >= 20, f"fixture has only {len(REPOS)} repos; it was captured with 23"
...
def test_no_confident_row_contradicts_a_repo_the_census_actually_read():
    ...
    assert hits >= 3, f"only {hits} confident rows scored; the rules may have stopped firing"
...
def test_ingest_accepts_both_shapes_a_search_returns():
    ...
    bare = ROOT / ".claude" / "probes" / "census-2026-09-01-ai-humanizer.json"
