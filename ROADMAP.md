...
| 13 | The per-language **catalogues** | 🔜 open — **the scaffold is now finished and self-enforcing; only the catalogues need a native speaker** | **people who speak those languages.** Korean 번역체 calques and Chinese academic-register tells are not guessable from outside. What WAS ours to finish: CONTRIBUTING required "a measurement, not a word list" and `register()` enforced nothing — any callable could register and report tells per 100 words with the same authority as English, whose every category carries a precision figure. Now `conforms()` checks the returned shape by calling the scorer (a missing `tells_per_100w` does not fail loudly — callers `.get` it and None prints as a blank where a rate belongs), `evidence=` records how precision was measured, and `unmeasured()` lists catalogues that state none. Deliberately not mandatory: refusing an unmeasured catalogue would push a contributor to invent a justification rather than state none. [How to add one](CONTRIBUTING.md#adding-a-language) |
| 14 | `pip install untell` claims the top-level names `eval` and `training` | 📦 open | **your decision, and here is the evidence for it.** ⚠️ **The collision is concrete, not theoretical: both names are taken on PyPI** — `eval` 0.0.5 ("Show value of an importable object") and `training` 0.0.1, VERIFIED by fetching each project's JSON. Installing untell shadows two published packages. Scope of the mechanical fix: **forty-one** Python files, **138 line-start imports** to rewrite. (Spelled out because `\d+\s+modules` is how the audit recognises a test-module claim, and a bare digit here reads as one — round 122 hit the same edge and this round hit it again.) ✗ Dropping them from the wheel instead is **not** an option that costs nothing: `untell/` imports them 0 times, but **four documented console scripts** point into `eval.*` — `untell-ceiling`, `untell-compare`, `untell-prove`, `untell-detector-audit` — so not shipping the package breaks all four. The real choice is the rename the row describes, which changes a published import surface; that is yours, not something to do in passing |
| 15 | Surrogate distillation | ⛔ blocked | a GPU — **verified absent**: `nvidia-smi` not on PATH and no `/dev/nvidia*` device nodes exist |
| 16 | RL-against-ensemble | ⛔ blocked | a GPU — **verified absent**, same probe as 15 |
| 17 | Alignment rewriter | ⛔ blocked | a GPU — **verified absent**, same probe as 15 |
| 18 | Calibrated verdict thresholds (conformal, length-conditioned) | ✅ done — `eval/calibrated_thresholds.py`, split-conformal per word-count band on the pre-ChatGPT corpus (human by construction, so no annotator). MEASURED at target FPR 5%, lite, n=500: the 50-100 word band moves **29.1% → 3.6%** false positives and 100-200 moves **15.8% → 4.8%**. ✗ **And that is not a win.** The sensitivity column, which any FPR-only table would omit, shows TPR already **9.3%/9.1%** at the fixed bar and **2.3%/0.0%** at the calibrated one: calibrating away the false positives removes what little detection existed. The finding is **this tier cannot support a verdict at any threshold**, which is stronger than "better thresholds" and visible only because both columns are reported | — |
| 19 | Assisted-arm loaders — Beemo, ARB, and the LREC resume corpus | 🔜 open — **the loader is built and generic; the three corpora cannot be fetched here** | **The blocker was never the loaders.** `load_rows` hard-coded the Pratama corpus's column names and `ARMS` hard-coded its five arm columns, so a second corpus could not be loaded **at all** without editing the module. A `CorpusSpec` now makes adding one a declaration, `validate()` names a missing column **before** anything is scored, and a test asserts the generic path reproduces the hard-coded one on the **real** corpus (72 rows = 72). ⚠️ Deliberately ships **no guessed schemas** for the three named corpora: `huggingface.co` returns 403 (VERIFIED), and a loader written against a schema nobody has inspected crashes on the real file or, worse, selects nothing and reports a clean arm. Whoever has the file declares a spec |
| 20 | The base-vs-instruct audit arm | 🔜 open | **the arm needs base-model and instruct-model OUTPUT, and generating either needs weights.** VERIFIED: `huggingface.co` is blocked (403) and no local weight cache exists — no `.safetensors` or `pytorch_model.bin` anywhere on disk. ⚠️ The "no new dependency" note was about the analysis, not the corpus; the corpus is the blocker and this row did not say so |
| 21 | Third-party watermark audit (Article 50 marking) | 🔜 open | a key-free third-party audit needs **watermarked text to audit**, which needs a watermarking model. VERIFIED unreachable by the same probe as 19 and 20. Blueprint in §7 stands; the measurement cannot run in this environment |
...
| 28 | Disability and neurodivergence as a fairness arm | ✅ **answered, and there IS a disparity.** The *Centering the Margins* route needs no subgroup label. Run on all **6,810** pre-ChatGPT documents — human by construction, so every flag is a false accusation — as a Cochran-Armitage trend across distance quintiles computed **inside** each word-count band: **z=6.55, p≈0, rising with distance, all 4 strata**. Text furthest from the corpus norm is falsely flagged significantly more often, and it survives the length control. ✗ The top-20% split this arm shipped with reports **no separating gap on the same data** (p=0.092) — its null was a **power artefact**, not a finding, and it had been the only statistic available. Converges with `eval/homogenization.py` (z=+3.91 against the machine centroid): two instruments, two different centres, one direction. ⚠️ Outlier status is not a protected characteristic — this is a measured disparity against **atypical writers**, which is the population the disability and L2 literatures are about but is not the same claim | — |
...
| 37 | An advisory caveat must not dominate the request it qualifies | ✅ done — `_mostly_locked_warning` was **87.8% of a 50,000-character `score_text` call on prose and 91.8% on a comma blob**. The note is a threshold on a proportion, so long input is sampled at 5,000 characters and re-read in full only near the bar. Warm — what a running server pays, once the spaCy model is loaded — **1.526s → 0.229s** on prose and **10.852s → 1.498s** on the worst shape, with the note unchanged on 425 documents | — |
...
| 46 | Say the false-positive rate at the length in front of you | ✅ done — the short-text warning stops at 40 words and the elevated rate does not: **28.69% of known-human 60–100-word documents are flagged at the shipped bar**, against 12.77% above 200, and none of them carried a caveat. Now they do, for that band only — 100–200 sits either side of the 19.47% average and a note there would be noise | — |
| 47 | Say whether short text is scored noisily or scored wrongly | ✅ done — the two imply different fixes, and it is **the mean, four to one**: matching the long band's mean closes **81%** of the flag gap, matching its spread **23%**. Short human text is read as more machine-like, not less legibly, so per-length thresholds are right and abstention would discard a real signal | — |
| 48 | Find what actually causes the length bias | ✅ done — a missing `− 1`. `_burstiness` is a coefficient of variation over sentence lengths and divides by `n`, so it reads **0.3778 at three sentences against a true 0.5000** — MEASURED on one fixed distribution, where any gradient is estimator bias. That is **0.116 of score** from sample size alone, larger than the 0.052 shift observed | — |
| 49 | Correct it — and do not ship the correction on half a measurement | ✅ done — `burstiness_bias_corrected` is present and tested, and the default is **unchanged**. Adopting it takes the pre-LLM false-positive rate **19.44% → 12.41%**, but it lowers every score, and detection power needs an AI-labelled corpus this environment cannot reach. A test fails if anyone wires it in first | — |
| 50 | Measure detection power where no labelled corpus is reachable | ✅ done — a language model wrote one. 70 abstracts matched to the human arm's register and lengths; the label is provenance, not annotation. `eval/detection_power.py` pools only where both arms have data | — |
| 51 | Report what the two arms actually say | ✅ done — **the detector flags 10.7% of machine abstracts and 30.4% of human ones** over the matched 40–100 range, intervals not overlapping, and flags human text more often in **every** band. On this register it is not weak, it is **reversed** — it measures genre conformity and calls it authorship | — |
| 52 | Say it threshold-free, so it is not a property of the bar | ✅ done — **AUROC 0.3529**, bootstrap CI **[0.2822, 0.4270]**, entirely below the 0.5 coin flip. It climbs toward 0.5 with length (0.1873 → 0.3599 → 0.4589), which is the small-sample burstiness mechanism showing in the outcome. ✗ And it retracts row 49's reading: the estimator correction is **0.3538 → 0.3402** on the component path, no gain in ordering — the flag-rate "improvement" was the bar moving, not the classes separating | — |
| 53 | Find which term inverts the detector — or that none does | ✅ done — none does. **Both live components are below 0.5** and burstiness (0.4122) is the *better* one; `common_signal` alone is **0.3459**, worse than the full score. Academic abstracts are dense in stock phrasing, so the feature reads genre and the genre here is the human arm. There is no term to drop, which is why correcting `_burstiness` moved nothing | — |
| 54 | The reversal must reach the pages people read | ✅ done — it lived in the ledger for three rounds. `README.md` and `docs/index.md` now carry the matched flag rates and **AUROC 0.3529** *above* the calibration answer, since that answer fixes the false-positive half of a detector whose other half had never been measured. Two figures stale since round 69 fixed on the way | — |
| 55 | Catch a number that is merely superseded, not retracted | ✅ done — the retraction guard searches for retired *forms* and nothing had been withdrawn, so `docs/index.md` published stale survey counts for ten rounds. `eval/data/survey_counts.json` makes the survey re-derivable without committing 186 XML files, and `check_survey_counts` reads documents against it. ⚠️ Its first version reported the **census** (1,718 volumes) as drift against the **sample** (186) — composite sentence patterns now keep the two apart | — |
| 56 | Measure the tell catalogue against machine text, as the score was | ✅ done — it had never been. **8.6% of machine abstracts carry a tell against 48.1% of human ones**, AUROC **0.2697** — further from a coin flip than the score's 0.3529 — and **all thirteen categories that fire, fire more on human text** | — |
| 57 | Report a tell count with the rate human writing has it | ✅ done — `ai_vocab` appears in **45.67%** of pre-ChatGPT ACL abstracts, `formulaic_transition` in **18.43%**, and `state-of-the-art` alone in **25.37%**. `score_tells` now returns the measured share beside the count. The catalogue is **not** edited: an NLP corpus is the worst evidence for removing the NLP term of art | — |
| 58 | Separate what the catalogue reads from what it claims to read | ✅ done — by holding authorship constant. Same model, same session: **0.092 tells/100w academic against 7.357 assistant** at matched length, AUROC **1.0000**, and **0.000 against 8.523** at 30–60 words. It cannot tell promotional from assistant apart (0.5625), which is what a register detector should do | — |
| 59 | State the fair account of both detectors | ✅ done — the catalogue separates **register** perfectly (1.0000) and **authorship** worse than a coin flip (0.2697); the lite score does neither (0.5095 and 0.3529). **A perfect register classifier used as an authorship classifier is worse than chance** whenever the arms differ more in register than in author | — |
| 60 | Measure the number a user actually reads | ✅ done — **a machine-written abstract is judged more human (80.8) than a real one (75.4)**, two times in three. The 0.978 AUROC already in `humanness.py` is HC3 — the assistant register — so the same score gives 0.978, then 0.6009 the right way, then **0.6733 the wrong way**, purely by register. Both figures now sit in the same comment | — |
| 61 | Make the whole detector arc one command | ✅ done — `python -m eval.detection_power --run --registers` builds both arms, scores them and prints flag rates, AUROC, the inversion verdict and the same-author register table. It bounds the human arm above as well as below, skips text the detector declines to score, and names the missing download rather than printing an empty table | — |
| 62 | Reconcile the published figure with what the command returns | ✅ done — the command printed **0.3529** where four documents said **0.3538**. Both are real: 0.3538 came from a reimplementation of the score's components, written to compare estimators. **The reproduction command is the authority**, so every figure standing for the shipped detector is now 0.3529 | — |
...
| 68 | Vary the filter that could actually break the claim | ✅ done — round 66 swept the corpus filter, but the ratio rests on a **13-paper row four alternatives long**, where eight missed papers move it by a third. Broadened in four meaning-preserving rungs: **13→21 papers**, ratio **7.5x–14.2x**, never near parity. The shipped pattern is the ladder's **most discriminating rung** (lift 7.1 vs 3.3–3.6) and its **lowest count** — so the honest row is 13–21, not 13 | — |
| 69 | Test the inversion's EXPLANATION without any machine text | ✅ done — the register account rested on **56 machine documents**; it is testable on **6,841 human ones**, where more standard-sounding writing should score more AI. **rho +0.0586, CI [+0.0357, +0.0842], all six length bands positive.** Supported in direction — and bounded by the same run: that is **0.34% of the score's variance**, so it explains the inversion's direction and not its size. ✗ I predicted main-conference papers would score highest and was wrong: workshop papers do, because they use **commoner** vocabulary | — |
| 70 | Count the unchosen numbers instead of sweeping them one at a time | ✅ done — **111 constants, 41 (37%) with no stated reason** (49 as first published; round 71 corrected the census). And the five that decide the stdlib score were **not constants at all**, just literals in an expression a census cannot see. Named (0 of 6,912 scores changed) and swept: **30 settings, none brings the AUROC above 0.5** — the inversion is not reachable from the calibration. ⚠️ The weights favour the weaker signal on this register, and the two scoring paths weight the same axis in opposite directions | — |
| 71 | Stop hand-picking which constants to sweep | ✅ done — round 70 counted 41 undefended constants and swept the five that looked important, which is the reflex the census existed to replace. Perturbing **all** of them: **0 of 35 move the published score**, with a positive control at **99.6%** so the zero means something. ✗ It also corrected round 70's own count — the census stopped at the first non-comment line, so **8 constants under a group comment were reported bare**; 49 → 41. ⚠️ 6 constants are unreachable by perturbation and are listed, not scored as zero | — |
| 72 | Ask whether a source actually contains the figure that cites it | ✅ done — attribution is not agreement, and round 62 found the gap by reading. ✗ The obvious mechanisation **fails structurally**: linking by proximity gave **15 contradictions, all false**, and three narrowings of scope moved the count by zero — a sentence may name a tool and quote a figure from elsewhere. An explicit registry ships instead: **19 figures, 19 verified**, no false positives possible. It found that the headline (10.7% / 30.4% / 0.3529) had **no committed artefact at all** | — |
| 73 | Ask it about other people's papers, where the stakes are higher | ✅ done — **33 cross-check findings, all read, none a misattribution.** ✗ The defect was that an earlier round's reading of 25 lived in prose, so when the count drifted to 35 nothing could tell a new finding from a cleared one. `eval/data/citation_triage.json` records a reason per finding, keyed so it survives edits; `--untriaged` exits non-zero on anything new. Two checker fixes: a row cross-reference and the digits inside `H2L` were both being read as figures | — |
| 74 | Ask whether the tests would notice a broken detector | ✅ done — 10,000+ tests and only the audit checks had ever been shown able to fail. **97 mutants, 45 killed, 52 survived: mutation score 46.4%**, both baselines clean. Survivors cluster in `_repetition_signal`, corroborating round 59's finding that it never fires. ⚠️ Against a **1,543-test** selection a sample of survivors dies 40% of the time [17%, 69%] — so the suite catches more than 46%, and **6 of 10 survive even that**. ✗✗ Two harness defects of mine: an exit code where the baseline is red, and a comparison against pytest's summary line including its elapsed time, which scored 10/10 killed when the truth was 4/10 | — |
| 75 | Stop hand-picking which modules to mutate | ✅ done — round 74 paired **two** modules with hand-written test selections, which is the reflex rounds 70–71 already caught. Selections are now derived from what each test imports, ranked by breadth. **108 mutants across 666 modules, 59 killed: 54.6%** — close to the hand-picked pair's 46.4%, so that pair was not unrepresentative. ✗ The first run reported 50.0% and was corrupted: a timeout sentinel of 10,000 as a **baseline** made every mutant for 666 modules an automatic survivor — round 71's lesson, in a harness written two rounds after it | — |
| 76 | Act on the survivor list instead of publishing it | ✅ done — a list nobody acts on is the defect rounds 72–73 kept finding. **Six survivors closed across two modules, 6 of 6 kills verified** by re-applying each mutant against only the new test. `dominant_script` returned the **rarest** script under `max→min` with the suite green; `html_report` **dropped every character after the last locked span** under `<→>=`. ✗ Two of my own assertions were wrong first — a guessed script name and a guessed crossover — in a round about tests that check nothing | — |
| 77 | Verify the kills, and catch the harness | ✅ done — a test written while looking at a mutant tends to assert what the mutant does not change, so each kill is re-verified. That caught **a defect in the harness**: CPython invalidates a `.pyc` on (mtime, size), every mutation here is a **same-size** single-character swap, and a write inside the same mtime second leaves stale bytecode valid — the mutated code never loads and the mutant is scored a survivor. One-directional, so kills stand and scores were under-estimates. **Round 75: 54.6% → 58.3%.** Round 74 re-ran unchanged, because its tests are slow enough that mtimes differ — **the bug bites hardest where mutation testing is cheap** | — |
| 78 | Check every other cache for the same defect | ✅ done — round 77 lost a measurement to an incomplete key, and `score.py` already carried a scar from the same shape (**56 assertions failed** before the torch mode went into the key). **6 cached functions audited; one has an empty key** — `human_base_rates()` takes no arguments and reads a file. Accepted and **guarded**: a check fails if any test patches a name a cached function reads without calling `cache_clear`. ✗ Its first version flagged the whole module and was wrong on its only finding — and I had written the coarse rule down as deliberate | — |
| 79 | Question the mutation operators, not just the modules | ✅ done — the harness had **three operators and nobody chose that set**. Added five (boundary, boolean, membership, identity, constant): candidates **1,397 → 3,313**. ✗ **Every operator I had implemented scores above every one I had not** — the original three kill 62.6%, the added five 39.8%, and the package score falls **58.3% → 46.2%**. Sharpest row: `<`→`<=` at **25%** against `<`→`>=` at **60%** on the same sites — the suite tests that branches do the right thing, not that they switch in the right place | — |
| 80 | Pair the two mutations at one site, unsampled | ✅ done — **339 comparison sites, both mutants at each. Of the 55 discordant pairs the off-by-one survives 55 times and the inversion 0** (exact binomial p = 5.6e-17): not a tendency, a total ordering. Killing an off-by-one implies killing the inversion, so `boundary` is strictly sharper and `comparison` strictly redundant beside it. ✗ Unsampled also corrects round 79's table — **comparison 60.0% → 35.4%, boundary 25.0% → 18.9%** — because `--limit` weights modules equally and an uncapped run weights them by size | — |
| 81 | Test the thresholds a person actually meets | ✅ done — round 80's finding predicted where it would bite, and **7 of the 8 comparisons guarding a documented threshold had a surviving off-by-one**, four with nothing testing either branch. Asserted at n−1, n and n+1: **7 of 7 now killed**, verified per mutant. ✗ The first pass killed only 4 — one sample straddled the boundary without touching it, one constant had a **second call site**, and `_MIN_WORDS_FOR_SIGNAL` is guarded **twice**, the copy in `.score()` untested | — |
| 82 | Find the leverage in 206 untested branches | ✅ done — largest concentration is **14 in one function**, whose docstring argues a checkable superset property nothing checked. One property test kills **6 of 28** mutants — real leverage, far short of complete, since a corpus exercises branches at typical values not boundaries. ✅ **Five of the six kills are inversions, one an off-by-one**: round 80's bias reproducing inside a test written by an author who knew about it. Property tests scale and are blind to boundaries; n−1/n/n+1 catches boundaries and does not scale | — |
| 83 | Generate the boundary list instead of remembering it | ✅ done — round 81 found **8** thresholds by grepping names I thought of; enumerating comparisons against named constants finds **48**, of which **30 have a surviving off-by-one** (37.5% protected). ✗✗ The register's first run said 7 protected, because `test_index` ranks by **breadth** and a boundary test imports the constant *and* its callers — so it ranks last and is dropped. Fixed: **18.9% → 25.4%**, **22 kills recovered from tests already written**. ✗ And a register is only as current as its sweep; a staleness guard now leads the report | — |
| 84 | Check the register before writing thirty tests | ✅ done — the harness has twice reported false survivors, so each of the 30 was re-run against **every test importing its module**, uncapped. **27 genuine, 3 selection artefacts — 90% accurate**, now measured rather than assumed. Four user-visible thresholds closed, **4 of 4 mutants killed**. ✗ I asserted the runner's voice floor must exceed the voice module's; it is **20 against 150** and the assertion was wrong — they answer different questions, and what needed testing was that a sample **inside the gap** still reaches its warning | — |
| 85 | Make the wrong-key failure findable, not just documented | ✅ done — this session guessed a return shape wrong **six times** despite `result-shapes.md` warning about exactly that. The existing check verifies returned keys are documented and is blind to a caller reading a key that never existed. The reverse check finds **8 undocumented conditional keys** — all on failure paths or non-default arguments — and a **tautology**, `assert X or True`, that could never fail. ✗✗✗ Its own first version gave **38 pairs, mostly false**; three fixes took it to 9, all verified | — |
| 86 | Write down how far to trust each checker | ✅ done — eight checkers and no way to tell which gate a commit or which have had their findings read. The register records both, plus each one's first-version defect: **7 of 8 shipped a version that was too loose**. Measured one of the two unmeasured — `constant_census` is **11 of 12** real on a seeded sample, and the sample exposed an ambiguity in its own definition (a reason for the mechanism is not a reason for the number). ✗ The register claimed `constant_influence` gates; it always exits 0, and its own test caught it | — |
| 87 | Measure the last unmeasured checker — the harness itself | ✅ done — the mutation harness had the most reason to be wrong (**both known defects produced false survivors**) and the least evidence. 24 survivors stratified across operators, each re-run against **every importing test**: 21 genuinely uncaught, **87.5% precision** [69.0%, 95.7%] — consistent with the boundary register's 90% measured the same way. Register now **8 of 8 measured**. ⚠️ Per-kind cells are 3 samples each and are not a ranking: a 2-of-3 cell spans [20.8%, 93.9%] | — |
| 88 | Measure what the checkers MISS, not just what they get right | ✅ done — every precision figure was obtained by reading findings, a method that cannot see anything a checker never reports. **22 defects planted across three checkers, each labelled easy or hard: 21 caught on the first run.** ✗ The miss was a **blind spot created by a precision fix** — round 85's scope pruning killed six false positives and silently blinded the checker to reads inside a closure. Fixed; **22 of 22 caught, still zero findings on the repo**. ✗ Also renamed a passing test whose name asserted the opposite of the design | — |
| 89 | Extend the plants to a gating checker, and distrust the result | ✅ done — `cache_keys` came back at **50% recall, missing the easy cases**, which for a gating checker is a headline. ✗ It was wrong: all three misses named the mutable global `_STATE`, and `"_STATE".isupper()` is True — the checker's own docstring reads upper-case as immutable, so **the plants contained no defect**. Rewritten: **6 of 6**. Precision is inflated by a false finding and recall **deflated by a false plant** — the same error mirrored. Each checker now has a **clean control** that must not fire, or the tool refuses to report | — |
| 90 | Land the fresh sweep the hook kept flagging | ✅ done — the register had run on data predating round 81's tests. Rebuilt: **protected 18 → 21, unprotected 30 → 26, share 37.5% → 44.7%**, with all four of round 84's closures confirmed. ✗ One module became **unmeasurable** as a cost of round 79's selection fix: `audit.py` now selects `test_every_audit_check_can_fail.py`, itself a mutation suite, and one baseline pass takes **4m27s**. Recorded, not fixed by a longer timeout — the fix bought +22 kills and cost one module's measurability, and neither was visible when it was made | — |
| 91 | Find the mechanism round 89 said it could not | ✅ done — a single plant gives two outcomes, so "the plant is empty" is indistinguishable from "the checker is blind". Every plant is now a **minimal edit of its own control**, giving four: detected, **empty plant**, dirty control, inverted. Round 89's exact plant re-run as a pair classifies as **empty, not as a miss**, and the tool refuses to report while any pair is broken. The `cache_keys` pairs differ **only in the case of the name** — the distinction that was got wrong — so the error is now unwritable. **28 pairs, 28 detected, 0 broken**; edits capped at 4 lines | — |
| 92 | Fix the humanizer on real machine text | ✅ done — run on 70 committed machine-written abstracts rather than the demo corpus, `--rewriter surgical` moved **0 of 10** documents (0.3025 → 0.3025, `stalled` on 6) while the default `composite` moved 5 and `structural` 5. Cause: it ranks words by tell removal, and `_tell_ranks` returns only `gain > 0`, so text with **no catalogued tell has an empty ranking** and the substitution loop never runs an iteration — **36 of 40** abstracts. Round 82 measured the catalogue as a register detector (1.0000) and an authorship detector (0.2697), so formal prose is precisely where it has nothing to offer. ✗ The real defect was that **nothing reported it**: an identical candidate passes every gate by construction, so `inspect` emitted `adopted` while `adopted` stayed **0**, and the note said "every draft scored worse" and advised `--best-of 3` — of a **deterministic** rewriter that had written no draft. The test meant to hold that note to a real run stubbed the rewriter to return its input, encoding the same conflation. Score-ranked fallback prototyped and **rejected on measurement**: 40/40 zero-substitution runs against 37/40 | — |
| 93 | Land the boundary sweep the hook had flagged since round 108 | ✅ done — **mutants 331 → 340, killed 86 → 88, unmeasurable modules 2 → 1**. The register's share fell **44.7% → 43.8% with no boundary losing protection**: exactly one moved, `audit.py:903 _MODULE_DRIFT`, out of `unmeasured` into `unprotected`. Round 90's rule paying out in the unbudgeted direction — the placeholder was flattering, and measuring made the number worse. ✗ **Why it became measurable is not a change anybody made**: same five-file selection (still including the 4m27s mutation suite), same 300s `--timeout`, opposite classification. Timed since: **206.46s solo** against round 107's 267s — not marginal alone, but **the sweep runs 4 workers in parallel**, so no baseline is ever measured in isolation and contention is the variable. The share should not be compared across rounds without that caveat. ✗ Round 109's own new boundary, `run.py:545 unchanged >= rewrites`, **survived** — and is a **selection artefact**: applied by hand it is killed **6 ways** by two test files, neither in `run.py`'s breadth-ranked ten. A test written for one branch is the least broad thing in the repo, so the ranking drops it exactly when it is the only one that matters | — |
| 94 | Test round 110's contention claim at `--workers 1` | ✅ done, result inconclusive — **the prediction is untested because the run never ran the comparison.** `--kinds boundary --workers 1` was killed at a **7-hour timeout with no output**, against ~50 minutes for the same sweep at 4 workers. Cause: the CLI passed `kinds` only inside the `workers > 1` branch and `run()` had no such parameter, so the serial path **accepted the flag, ignored it, and swept every operator** — the 1,397-candidate job, for a caller who asked for 340. Nothing warned. Fixed, with the empty-candidate skip and filter-before-cap ordering `_worker` already had. ✗ The first test written for it was **vacuous** — it reproduced the narrowing inside the test and passed pre-fix; caught by a positive control, not by reading. All 4 now fail pre-fix | — |
| 95 | Land both sweeps and compare them | ✅ done — three sweeps, two worker counts, **340/88/252 and 25.9% in all three**, identical survivor sets, identical 44 baselines; the fresh 4-worker run reproduced the committed artefact **to the byte**, so neither it nor the register needed changing. The sweep is **deterministic and worker-count invariant** at the mutant level. The staleness hook compares mtimes, and the under-reporting it warned about was **exactly zero** here — read "stale" as "unverified", not "wrong". ⚠️ **Round 110's prediction remains untested**: `audit.py` came back measurable in all three, so round 107's `unmeasurable` outcome failed to reproduce and the control arm never exhibited the condition — round 108's dirty-control rule applied to a prediction. Serial artefact committed for checkability; it predates the shape fix and carries no `outcomes` key. ✗ **Contention then tested directly and REFUTED**: four concurrent copies of `audit.py`'s selection ran **177/178/179/180s**, every one *faster* than the 206.46s solo figure, against a 300s cut. Round 110's explanation is wrong and its 206s baseline was the slow observation, taken under unrecorded load. Round 107's timeout now has **no** explanation; cold `__pycache__` in the sweep's fresh worktrees is the remaining harness-side candidate | — |
| 96 | Explain the flip, or fix what makes it cost anything | ✅ done — cold worktree measured at **113s**, the *fastest* of every condition (vs 177-180s contended, 206s warm solo, 300s cut), so the last hypothesis is refuted too and round 107's timeout has no surviving explanation. ✗ The harness fault is the real finding: `_failures` returned the **same `UNUSABLE`** for a timeout and a collect failure, so the record could only say "times out **or** fails to collect" — two causes, opposite remedies — and nothing retried. Split into `UNUSABLE`/`TIMED_OUT`, retry once on timeout only, cause recorded: the first sweep under the fix shows `fast_detectgpt | collect_error | retried: False`, so it **never timed out** and every artefact has half-misdescribed it. ✗ The split nearly broke `verify_survivors`, where `== UNUSABLE` misses `-2` and would have scored **every survivor "killed by the wider suite"** — precision reported better than truth | — |
| 97 | Fill *Gaps worth noting* #5 — FPR against distance from the machine centre | ✅ done — and **the answer is that the question is under-specified**. On **6,810** pre-ChatGPT ACL abstracts (human by construction, so every flag is a false positive) the length-stratified trend is a **smooth monotone crossover in the vocabulary constant, significant at both ends in opposite directions** — z = +3.91, +3.70, +2.03, +0.50, −0.41, −2.49, −3.70, −5.02, −4.30 across sizes 30→800. **On the stylistic axis the gap names, the prediction is backwards**: in function-word space false positives **RISE** with distance from the machine centre (z=+3.91, p=0.0001), falling only in the content-inclusive space (z=−5.02) which is substantially topic. Detectors flag a writer for departing from the reference human distribution, not for resembling the model. ✗ **A "refuted, z=0.08 p=0.93" headline was published from vocabulary 150 alone and retracted within the hour by this round's own sweep** — 150 is exactly where the curve crosses zero, so it reads as "no effect" and means "the sign changes here". Rounds 86 and 89 again, committed by the round that cites them as its guard. ✗ Four sweep points bracketed the crossover without revealing it was one; the default is now nine spanning 30-800. ✗ **The naive study returns a significant result in the OPPOSITE direction**: unstratified, z=2.16, **p=0.031** for false positives rising with distance, which reads as "being unusual protects you". Pure length confound — farthest quintile 130 words against nearest 172. The gap is not merely unfilled, it is booby-trapped. ✗ Displacement arm: **no rewriter moves machine text off the centroid** (p=0.29/0.73/0.29/1.00) while the score falls 0.3049→0.2684, and all three working rewriters significantly homogenize **human** text toward it — the loop's gains are detector-specific, not stylometric | — |
| 98 | Measure every technique class the census names, not three of them | ✅ done — `eval/technique_matrix.py` runs every class that can run here on one corpus against **four** axes (detector score, catalogued tells, stylometric displacement, output integrity), against the shipped comparison's three techniques and one axis. Three results a single-axis table cannot show: **unicode trickery moves P(AI) by 0.0000** while planting **641** invisible or counterfeit characters (7 census repos build on it); **word substitution changes 0 of 25** documents (39 repos); and **every technique that works has negative Δstyle** — all move TOWARD the machine centroid while lowering the score, the in-loop/held-out gap made concrete. ✗ First run reported back-translation as ineffective when it was **never installed** — availability is now probed, not inferred. Synthesised in `docs/removing-ai-tells.md` | — |
| 99 | Land the stale boundary sweep | ✅ done — **killed 88 → 89, score 25.9% → 26.2%**. ⚠️ **The register figures first published here (protected 21 → 22, share 43.8% → 45.8%) are withdrawn**: that sweep ran 21:14–22:09 and `--fix-counts` landed at 21:19, so worker worktrees created at staggered times saw different documents and the run measured a tree changing underneath it. Re-run on a stable tree: same totals, **protected 21, share 43.8%**, and the killed mutant is `audit.py:899` not `:903`. ✗ The one mutant that flipped is the finding: a boundary in `audit.py` is protected — and **no test changed to protect it**. (VERIFIED by hand: the line is **`:899`**, `claimed > len(modules)`, where `>=` fires on a correct document; `:903` stays genuinely unprotected because the test plants a claim of three against 665 and `662 > 5` equals `662 >= 5`.) A number in three Markdown files did. `test_test_inventory` opens with `assert_passes("check_test_inventory")`; this session's six new test files drifted the docs' module count 659 vs 665 past a tolerance of 5, so the precondition failed, the test **aborted before reaching the boundary**, and the mutant survived because the test covering it could not start. ✗ **The exposure is 20 tests** — every `assert_passes(` in that file is the same shape. ⚠️ A mutation sweep is only valid on a tree whose own guards pass; rounds 110-113 were all taken while this drift was live | — |
| 100 | Explain why `audit.py:899` keeps changing status | ✅ done — **it was never flakiness.** The mutant is `claimed > len(modules)` → `>=`, and `test_test_inventory` opens by asserting the unmutated check passes. With counts **exactly** equal `666 >= 666` fires and the mutant is **killed**; off by one, `665 >= 666` is false, nothing observable changes and it **survives**. VERIFIED under both conditions by hand. ⚠️ A drift of one is **deliberately allowed** — `_MODULE_DRIFT = 5`, for a documented and correct reason — so adding a test file unpins the boundary and **no guard complains**. ✗ This refines round 122: "a sweep is only valid on a tree whose guards pass" is necessary and **not sufficient**. Every guard passed here. A mutation score is a function of the tree's incidental state, not only of its tests. ✅ **Tested by prediction and confirmed**: with the counts committed, the account predicted **89 killed and 899 dead** before the run, and the sweep returned exactly that (340/89/251, 26.2%). ✗ It took three attempts to run that test correctly — 117 changed the tree during a sweep, 122 re-ran cleanly, and this round left `--fix-counts` **uncommitted** while the sweep built its worktrees from **HEAD**. The surviving rule is narrower: **a sweep measures the committed tree at the moment its worktrees are created, and nothing else** | — |
| 101 | Test the L2 hypothesis rounds 114 and 120 refused to assert | ✅ done — **and it is directionally right and not significant.** Both rounds found false positives rising with function-word distance and both declined to claim it explains the L2 result, for want of a corpus with author status. The Pratama corpus has one — **36 Native, 36 Non-Native**, balanced, reachable on GitHub where HF is not. Permutation test: non-native sit **+0.0394** further from the corpus centre (**p=0.098**), **+0.0441** length-matched (**p=0.066**). Same direction in both arms, neither clears 0.05; the effect size implies **~79-104 per group** to settle, against 36. ✅ Length is **not** the confound this time — median 176 vs 180 — which matters because length reversed the sign of this study once already. ⚠️ In this corpus **native** writers were flagged more (13.9% vs 5.6%), opposite to the literature, on **7 flags total** — reported because omitting an inconvenient direction is how a null becomes a claim | — |
| 102 | Re-run the PubMed pass; the survey was a snapshot | ✅ done — two relevant papers published since round 4, neither cited. **Villa et al.** report **0 false positives among 30 human texts** on full-length dentistry manuscripts, six detectors — which sits oddly beside the **61.3%** TOEFL row until length reconciles them, and length is the axis this repo measures directly (**28.69%** at 60-100 words vs **12.77%** above 200). Two correct studies, opposite ends of one curve. ✗ **Du et al.** gave **214 teachers** the *same* paper with a fictitious detection report varying only 7% vs 87%: the high rate lowered their judgements of originality, language expression and logical structure — of writing that had not changed. First external evidence that **presentation changes the verdict**, which converts three of this repo's reporting choices from taste into requirements. ⚠️ Two papers, one database; PubMed does not index CS, and ACL/arXiv are egress-blocked | — |
| 103 | Audit round 102's own claim about our reporting | ✗→✅ **the claim was an overclaim, now fixed.** Round 102 said Du et al. converts three of this repo's reporting choices into requirements, listing "refusing to print a verdict a tier cannot support". Checked by **running the tool**: (a) FPR beside the flag ✅, (b) verdict withheld ❌ — `flagged: True`, `ai_percent: 46.1` on the stdlib path, (c) sensitivity beside thresholds ✅. Round 116 *concluded* the tier supports no verdict and deliberately changed nothing, which is a defensible refusal to break a public field — not the same as having made the choice. ✅ Fixed **additively**: `verdict_supported`, a boolean a caller can act on instead of parsing English, with `flagged` and `ai_percent` unchanged and every consumer still working | — |
