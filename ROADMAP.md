...
| 37 | An advisory caveat must not dominate the request it qualifies | ✅ done — `_mostly_locked_warning` was **87.8% of a 50,000-character `score_text` call on prose and 91.8% on a comma blob**. The note is a threshold on a proportion, so long input is sampled at 5,000 characters and re-read in full only near the bar: **3.88s → 1.20s** and **13.71s → 1.48s**, with the note unchanged on 425 documents | — |
