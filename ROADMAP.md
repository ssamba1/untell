...
| 99 | Land the stale boundary sweep | ✅ done — **killed 88 → 89, score 25.9% → 26.2%, register protected 21 → 22, share 43.8% → 45.8%**. ✗ The one mutant that flipped is the finding: `audit.py:903 _MODULE_DRIFT`, the boundary round 110 spent three rounds explaining, is protected — and **no test changed to protect it**. A number in three Markdown files did. `test_test_inventory` opens with `assert_passes("check_test_inventory")`; this session's six new test files drifted the docs' module count 659 vs 665 past a tolerance of 5, so the precondition failed, the test **aborted before reaching the boundary**, and the mutant survived because the test covering it could not start. ✗ **The exposure is 20 tests** — every `assert_passes(` in that file is the same shape. ⚠️ A mutation sweep is only valid on a tree whose own guards pass; rounds 110-113 were all taken while this drift was live | — |
