...
| 28 | Disability and neurodivergence as a fairness arm | ✅ run — and recorded as run rather than as answered. The *Centering the Margins* route needs no subgroup label, and row 30 built it; this runs it on all **6,810** documents, stratified by length. **0 of 4 bands separate their intervals** and the sign reverses in one (60-100: 36.7% margin vs 26.7% centre, **+10.0pp**; 100-150: −1.3pp; 150-220: +4.2pp; 220+: +2.0pp). ⚠️ **Not evidence of no disparity — a power limit.** Pooled, margin 21.0% vs centre 19.0%, z=1.69, **p=0.092**: the same direction as `eval/homogenization.py`'s function-word result, which DOES separate (z=+3.91, **p=0.0001**) because a 5-quintile trend test has power a top-20% binary split does not. Two instruments, two different centres, one direction — text further from a norm is flagged more often. The largest gap sits in the shortest band, where power is lowest and this tool's error is highest. Next step written down: give the arm the trend statistic | — |
