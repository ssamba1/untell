...
| 19 | Assisted-arm loaders — Beemo, ARB, and the LREC resume corpus | 🔜 open | **the corpora are public and this environment cannot fetch them.** VERIFIED: `huggingface.co/datasets/toloka/beemo` returns **000 — CONNECT tunnel refused, 403** at the proxy, an organization egress policy. `raw.githubusercontent.com` answers 301, so GitHub-hosted data is reachable and HF-hosted data is not. Buildable wherever HF is reachable; the loaders are not the blocker |
