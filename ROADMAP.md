...
| 75 | Stop hand-picking which modules to mutate | ✅ done — round 74 paired **two** modules with hand-written test selections, which is the reflex rounds 70–71 already caught. Selections are now derived from what each test imports, ranked by breadth. **108 mutants across 647 modules, 59 killed: 54.6%** — close to the hand-picked pair's 46.4%, so that pair was not unrepresentative. ✗ The first run reported 50.0% and was corrupted: a timeout sentinel of 10,000 as a **baseline** made every mutant for 647 modules an automatic survivor — round 71's lesson, in a harness written two rounds after it | — |
