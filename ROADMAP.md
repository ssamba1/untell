...
| 52 | Say it threshold-free, so it is not a property of the bar | ✅ done — **AUROC 0.3538**, bootstrap CI **[0.2824, 0.4272]**, entirely below the 0.5 coin flip. It climbs toward 0.5 with length (0.1873 → 0.3599 → 0.4589), which is the small-sample burstiness mechanism showing in the outcome. ✗ And it retracts row 49's reading: the estimator correction is **0.3538 → 0.3402**, no gain in ordering — the flag-rate "improvement" was the bar moving, not the classes separating | — |
...
| 54 | The reversal must reach the pages people read | ✅ done — it lived in the ledger for three rounds. `README.md` and `docs/index.md` now carry the matched flag rates and **AUROC 0.3538** *above* the calibration answer, since that answer fixes the false-positive half of a detector whose other half had never been measured. Two figures stale since round 69 fixed on the way | — |
...
| 56 | Measure the tell catalogue against machine text, as the score was | ✅ done — it had never been. **8.6% of machine abstracts carry a tell against 48.1% of human ones**, AUROC **0.2697** — further from a coin flip than the score's 0.3538 — and **all thirteen categories that fire, fire more on human text** | — |
...
| 59 | State the fair account of both detectors | ✅ done — the catalogue separates **register** perfectly (1.0000) and **authorship** worse than a coin flip (0.2697); the lite score does neither (0.5095 and 0.3538). **A perfect register classifier used as an authorship classifier is worse than chance** whenever the arms differ more in register than in author | — |
