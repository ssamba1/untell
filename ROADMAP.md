...
| 97 | Fill *Gaps worth noting* #5 — FPR against distance from the machine centre | ✅ done — and **the answer is that the question is under-specified**. On **6,810** pre-ChatGPT ACL abstracts (human by construction, so every flag is a false positive) the length-stratified trend **flips sign with the vocabulary constant and is significant at both ends**: **z=+3.70, p=0.0002** (false positives rise with distance) at a 50-word feature space, **z=−3.70, p=0.0002** (they fall) at 300, null at 100 and 150. At 50 the features are function words; at 300 they are partly topical — two constructs sharing a name. ✗ **A "refuted, z=0.08 p=0.93" headline was published from vocabulary 150 alone and retracted within the hour by this round's own sweep** — 150 sits at the crossover, so it reads as "no effect" and means "the effect changes sign here". Rounds 86 and 89 again, committed by the round that cites them as its guard. ✗ **The naive study returns a significant result in the OPPOSITE direction**: unstratified, z=2.16, **p=0.031** for false positives rising with distance, which reads as "being unusual protects you". Pure length confound — farthest quintile 130 words against nearest 172. The gap is not merely unfilled, it is booby-trapped. ✗ Displacement arm: **no rewriter moves machine text off the centroid** (p=0.29/0.73/0.29/1.00) while the score falls 0.3049→0.2684, and all three working rewriters significantly homogenize **human** text toward it — the loop's gains are detector-specific, not stylometric | — |
